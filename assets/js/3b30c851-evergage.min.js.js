...
const SPECIALITY_SLUGS = [
  ...
  "internal-medicine",
  "lung-transplant",
  "minimal-access-surgical-gastroenterologist",
  "nephrology",
  ...
  "pathology",
  "pediatrics",
  "plastic-and-breast-reconstruction",
  "precision-oncology",
...
const normalizePhone = (phoneInput) => {
  ...
  for (let prefixLen = 3; prefixLen >= 1; prefixLen--) {
    const prefix = phone.substring(0, prefixLen);
    const rule = DIAL_CODES[prefix];

    if (rule) {
      const nationalLength = phone.length - prefixLen;

      if (rule.len.includes(nationalLength)) {
        locale = rule.iso;
        break;
      }
    }
...
function getDoctorDetailInfo() {
  ...
  var doctorCard = document.querySelector("[id^='doctor-']");
  ...
  var doctorId = (doctorCard.id || "").replace("doctor-", "");

  var hospitalId = (doctorCard.getAttribute("data-hospitalid") || "").replace(
    "hospital-",
    "",
  );

  var specialityId = (
    doctorCard.getAttribute("data-specialityid") || ""
  ).replace("speciality-", "");

  var cityId = (doctorCard.getAttribute("data-cityid") || "").replace(
    "city-",
    "",
  );
  ...
  var doctorName = getText(".doctor_d_name_inner");

  // Speciality

  var speciality = getText(".doctor_d_desc")
    .replace(/\s*:\s*$/, "")
    .replace(/&/g, "and")
    .replace(/\s+/g, " ")
    .trim();
  ...
  var qualification = getText(".doctor_d_educ")
  ...
  var languageContainer = document.querySelector(".doctor_d_lang");

  if (languageContainer) {
    var languageSpans = languageContainer.querySelectorAll("span");

    for (var i = 0; i < languageSpans.length; i++) {
      var language = languageSpans[i].innerText.replace(/\s+/g, " ").trim();
  ...
  var experienceText = getText(".doctor_d_exper");

  var yearsOfExperience = 0;

  var experienceMatch = experienceText.match(/\d+/);

  if (experienceMatch) {
    yearsOfExperience = parseInt(experienceMatch[0], 10);
  }

  // Consultation Details

  var timingText = getText(".doctor_d_timing");

  var consultationHours = "";

  var consultationDays = "";

  var timingMatch = timingText.match(/(\d{1,2}:\d{2}\s*-\s*\d{1,2}:\d{2})/);

  if (timingMatch) {
    consultationHours = timingMatch[1];

    consultationDays = timingText.replace(consultationHours, "").trim();
  }
  ...
  var imageElement = document.querySelector(".doctor_image_top_img");
  ...
  var hospitalRadio = document.querySelector(
    "input[name='doctors_page_city']:checked",
  );

  if (hospitalRadio) {
    hospitalName = hospitalRadio.getAttribute("data-hospital-name") || "";

    hospitalAddress = hospitalRadio.getAttribute("data-hospital-address") || "";
  }
  ...
  var cityContainer = document.querySelector("#select2-edit-city-container");

  if (cityContainer) {
    var selectedCity = cityContainer.innerText.replace("×", "").trim();

    if (selectedCity && selectedCity.toLowerCase() !== "select city") {
      city = selectedCity;
    }
  }
  ...
  // Fallback 2 - Hospital Name

  if (!city && hospitalName) {
    var hospitalParts = hospitalName.split(",");

    if (hospitalParts.length > 1) {
      city = hospitalParts[hospitalParts.length - 1].trim();
    }
  }
  ...
  return {
    ...
    doctorUrl: window.location.href,
  };
}
// ─── URL-KEYED MEMOIZED WRAPPER ───

let __doctorDetailCache = null;
let __doctorDetailCacheUrl = null;

function _getDoctorDetailCached() {
  const currentUrl = window.location.href;

  // Re-scrape if the URL changed OR if the current cache is empty/failed
  if (
    __doctorDetailCacheUrl !== currentUrl ||
    !__doctorDetailCache ||
    !__doctorDetailCache.doctorId
  ) {
    const freshData = getDoctorDetailInfo();

    // Only lock the cache and URL if we successfully grabbed real data from the DOM
    if (freshData && freshData.doctorId) {
      __doctorDetailCache = freshData;
      __doctorDetailCacheUrl = currentUrl;
    } else {
      // Return the null/incomplete object for this millisecond tick without locking the cache,
...
const getDoctorInfo = function (doctorWrapper) {
  // Experience Cleanup
  const doctorCard = doctorWrapper.querySelector(".doctor-card");

  var experienceElement = doctorWrapper.querySelector(
    ".expert_care_doctor_experience span",
  );

  var rawExperience = "";

  if (experienceElement) {
    rawExperience = experienceElement.textContent.replace(/\s+/g, " ").trim();
  }

  var experienceMatch = rawExperience.match(/\d+\+?\s*Years/);

  var experience = 0;

  if (experienceMatch) {
    experience = parseInt(experienceMatch[0].replace(/\D/g, ""), 10);
  }
  ...
  var imageElement = doctorWrapper.querySelector(".expert_image");
  ...
  var doctorNameElement = doctorWrapper.querySelector(".expert_doctor_name a");
  ...
  // var specialityElement = doctorCard.querySelector(".expert_care_doctor_desc");

  // if (specialityElement) {
  //   speciality = specialityElement.innerText.trim();
  // }
  ...
  var specialityElement = Array.from(
    doctorWrapper.querySelectorAll(".expert_care_doctor_desc"),
  ).find(function (element) {
    return !element.classList.contains("doctor_designation");
  });
  ...
  var hospitalElement = doctorWrapper.querySelector(
    ".expert_care_location_title",
  );

  if (hospitalElement) {
    hospital = hospitalElement.innerText.trim();
  }
  ...
  // var urlElement = doctorWrapper.querySelector(".expert_doctor_name a");
  var urlElement = doctorWrapper.querySelector(".expert_doctor_name a");

  if (urlElement) {
    doctorUrl = urlElement.href || "";
  }
  console.log("doctorUrl inside helper:", doctorUrl);
  // Return Doctor Object

  return {
    id: (doctorCard.getAttribute("id") || "").replace("doctor-", ""),

    hospitalId: (doctorCard.getAttribute("data-hospitalid") || "").replace(
      "hospital-",
      "",
    ),

    specialityId: (doctorCard.getAttribute("data-specialityid") || "").replace(
      "speciality-",
      "",
    ),

    cityId: (doctorCard.getAttribute("data-cityid") || "").replace("city-", ""),
...
function getLanguage() {
  ...
  const pathMatch = window.location.pathname.match(
    /^\/(hi|te|ta|kn|ml|bn|mr|gu|pa|or|en)(\/|$)/,
  );
...
SalesforceInteractions.init({
  ...
  document.addEventListener(
    SalesforceInteractions.CustomEvents.OnBeforeEventSend,
    (evt) => {
      console.log("🔥 MCP EVENT:", evt.detail);
    },
  );
  ...
  document.addEventListener(
    "click",
    (event) => {
      const ctaLink = event.target.closest("a.grid-card__full-cta");
      ...
      const card = ctaLink.closest(".grid-card") || ctaLink.parentElement;

      const ctaText =
        card.querySelector(".grid-card__cta .text").innerText.trim() ||
        ctaLink.innerText.trim() ||
        href;
      ...
      SalesforceInteractions.sendEvent({
        ...
        user: {
          attributes: {
            lastHomepageCTA: ctaText,

            lastPageVisited: window.location.href,

            lastPageType: getSafePageType(),
  ...
  (function () {
    ...
    document.addEventListener("input", (e) => {
      if (e.target.id === "edit-names") {
        savedName = e.target.value.trim();
      }

      if (e.target.id === "edit-phones") {
        savedPhone = e.target.value.trim();
      }
    });
    ...
    setInterval(() => {
      ...
      const msgEl = document.querySelector(".success-messages");

      if (!msgEl) return;

      const text = (msgEl.innerText || "").toLowerCase();

      if (!text.includes("request submitted successfully")) {
        return;
      }
      ...
      const phoneData = normalizePhone(savedPhone);

      console.log("Phone Data:", phoneData);

      SalesforceInteractions.sendEvent({
        interaction: {
          name: "Callback Form Submitted",
        },
  ...
  // ─── Flow A — Appointment Confirmed (fires on the reloaded page) ───
  ...
  (function () {
    ...
    // Safety net: if the success message never renders, don't leave the stash stuck.
    const giveUp = setTimeout(clearPending, 60000);
    ...
    SalesforceInteractions.DisplayUtils.pageElementLoaded(
      ...
      const msgEl = document.querySelector(".status-messages.messages--status");
      ...
      if (text.indexOf("thank you for sharing your details") > -1) {
        const phoneData = normalizePhone(pending.phone);
        const nameParts = (pending.name || "").split(/\s+/);

        SalesforceInteractions.sendEvent({
          interaction: { name: "Appointment Confirmed" },
          user: {
            identities: { phoneNumber: phoneData.phoneNumber },
            attributes: {
              firstName: nameParts[0] || "",
              lastName: nameParts.slice(1).join(" ") || "",
              ...
              phoneNumber: phoneData.phoneNumber,
              locale: phoneData.locale,
              // language / lastPageVisited / lastPageType are added by global.onActionEvent
            },
          },
        });
      }

      // Matched or not, this stash is spent.
      clearPending();
    });
  })();
  ...
  const sitemapConfig = {
    global: {
      contentZones: [{ name: "global_exit_intent_popup" }],

      onActionEvent: (actionEvent) => {
        actionEvent.user = actionEvent.user || {};
        actionEvent.user.attributes = actionEvent.user.attributes || {};

        actionEvent.user.attributes.lastPageVisited = window.location.href;

        actionEvent.user.attributes.lastPageType = getSafePageType();

        actionEvent.user.attributes.language = getSafeLanguage();
    ...
    pageTypes: [
      ...
      {
        name: "doctor_detail_page",

        isMatch: () => {
          const pathParts = window.location.pathname.split("/").filter(Boolean);

          const urlMatches =
            pathParts[0] === "doctors" && pathParts.length === 4;
          ...
          catalogObject: {
            ...
            id: () => {
              return SalesforceInteractions.DisplayUtils.pageElementLoaded(
                ...
                const d = _getDoctorDetailCached();
                return d ? d.doctorId : "";
              });
            },
            ...
            attributes: {
              ...
              url: () => {
                const d = _getDoctorDetailCached();
                return d ? d.doctorUrl : window.location.href;
              },
              imageUrl: () => {
                const d = _getDoctorDetailCached();
                return d ? d.imageUrl : "";
              ...
              specialization: () => {
                const d = _getDoctorDetailCached();
                return d ? d.speciality : "";
              },
              yearsOfExperience: () => {
                const d = _getDoctorDetailCached();
                return d ? d.yearsOfExperience : 0;
              },
              consultationDays: () => {
                const d = _getDoctorDetailCached();
                return d ? d.consultationDays : "";
              },
              consultationHours: () => {
                const d = _getDoctorDetailCached();
                return d ? d.consultationHours : "";
              },
              qualification: () => {
                const d = _getDoctorDetailCached();
                return d ? d.qualification : "";
              },

              languages: () => {
                const d = _getDoctorDetailCached();
                return d && d.languages ? d.languages : [];
              },
              location: () => {
                const d = _getDoctorDetailCached();
                return {
            ...
            relatedCatalogObjects: {
              Keyword: () => {
                const d = _getDoctorDetailCached();
                return d && d.specialityId ? [d.specialityId] : [];
              },
              Hospitals: () => {
                const d = _getDoctorDetailCached();
                return d && d.hospitalId ? [d.hospitalId] : [];
              },
            },
          },
        },

        // OTP / appointment listeners (Flow A) — left for the next pass, as agreed.
        listeners: (() => {
          return [
            // =====================================
            // Listener 1
            ...
            //   "#edit-submit-request",
            //   () => {
            //     console.log("Appointment Request Started");

            //     SalesforceInteractions.sendEvent({
            //       interaction: {
            //         name: "Appointment Request Started",
            //       },
            //     });
            //   },
            // ),
                ...
                if (d && d.doctorId) {
                  requestEvent.user = {
                    attributes: {
                      ...
                      appointmentDoctorName: d.doctorName || "",
                    },
                  };
                }

                SalesforceInteractions.sendEvent(requestEvent);
              },
            ),
            ...
            SalesforceInteractions.listener(
              ...
              () => {
                ...
                // #edit-submit-final reloads the page, so we can't watch for the success
                ...
                try {
                  ...
                  console.log("sessionStorage unavailable:", e);
                }
              },
            ),
          ];
...

      {
        ...
        interaction: {
          name: "Doctor Listing Page",
        },

        listeners: [
          SalesforceInteractions.listener(
            "click",
            ".card_common_btn.card_common_btn_yellow",
            (event) => {
              const appointmentBtn = event.target.closest(
                ".card_common_btn.card_common_btn_yellow",
              );

              if (!appointmentBtn) return;

              // const doctorCard = appointmentBtn.closest(".doctor-card");
              const doctorWrapper = appointmentBtn.closest(".doctor-card-view");
              ...
              // const doctorInfo = getDoctorInfo(doctorCard);
              const doctorInfo = getDoctorInfo(doctorWrapper);
              console.log("Doctor Info", doctorInfo);

              // Resolve City Name From URL

              let readableCityName = "";

              if (doctorInfo.url) {
                const url = new URL(doctorInfo.url);

                const pathParts = url.pathname.split("/").filter(Boolean);

                if (pathParts.length >= 4) {
                  readableCityName = pathParts[2]
                    .split("-")
                    .map(function (word) {
                      return word.charAt(0).toUpperCase() + word.slice(1);
                    })
              ...
              // 1. FIRE STANDARD EVENT: Updates the Doctor Catalog & builds User Affinity
              SalesforceInteractions.sendEvent({
                interaction: {
                  name: SalesforceInteractions.CatalogObjectInteractionName
                    .ViewCatalogObject,
                  catalogObject: {
                    ...
                    attributes: {
                      ...
                      // THE LOCATION OBJECT
                      location: {
                        city: readableCityName,
                        countryCode: "in",
                      },
                    },

                    relatedCatalogObjects: {
                      Keyword: [doctorInfo.specialityId],
                      Hospitals: [doctorInfo.hospitalId],
                    },
                  },
                },
                user: {
                  attributes: {
                    appointmentDoctorId: doctorInfo.id,
                    appointmentDoctorName: doctorInfo.name,
                    language: getSafeLanguage(),
                    lastPageVisited: window.location.href,
                    lastPageType: "doctors_page",
                  },
                },
              });

              // Speciality Catalog Affinity
              ...
              //       id: doctorInfo.specialityId,

              //       attributes: {
              //         name: doctorInfo.speciality,
              //       },
              //     },
              //   },
          ...
          SalesforceInteractions.listener(
            "click",
            "[data-drupal-selector='edit-submit-request']",
            function () {
              console.log("Appointment Request Started");
          ...
          SalesforceInteractions.listener(
            "click",
            "[data-drupal-selector='edit-submit-final']",
            function (event) {
              console.log("Submit Clicked");

              // Get the active form that contains the clicked button
              ...
              var nameElement = form.querySelector(
                "[data-drupal-selector='edit-name']",
              );

              if (nameElement) {
                submittedName = (nameElement.value || "").trim();
              }

              var phoneElement = form.querySelector(
                "[data-drupal-selector='edit-phone']",
              );

              if (phoneElement) {
                submittedPhone = (phoneElement.value || "").trim();
              }
      ...
      {
        name: "hospitals_city_filter_page",
        ...
        interaction: {
          name: "Hospitals City Filter Page View",
        },
      },

      {
        ...
        interaction: {
          name: "Find Hospital Page",
        },
      },
      /*Check this page */
      {
        name: "pro_health_packages_page",
        ...
        interaction: {
          name: "Health Package Listing Page",
        },
      },
      /*Check This page:  */
      {
        name: "book_second_opinion_page",
        ...
        interaction: {
          name: "Book Second Opinion Page View",
        },
      },

      {
        name: "health_library_detail_page",
...

      {
        ...
        interaction: {
          name: "Health Library Page View",
        },
      },

      {
        name: "centres_of_excellence_page",
        ...
        interaction: {
          name: "Centres of Excellence Page View",
        },
      },

      {
        name: "speciality_detail_page",
        ...
        interaction: {
          name: "Speciality Detail Page View",
        },
      },

      {
        name: "speciality_page",
        ...
        interaction: {
          name: "Speciality Page View",
        },
      },

      {
        name: "treatment_detail_page",
        ...
        interaction: {
          name: "Treatment And Procedure Page",
        },
      },
      ...
      {
        name: "health_package_detail_page",
        ...
        interaction: {
          name: "Health Package Detail Page View",
        },
      },
...

      {
        ...
        interaction: {
          name: "Medicine Detail Page View",
        },
      },

      {
        ...
        interaction: {
          name: "Medicines Page View",
        },
      },

      {
        name: "diagnostics_investigations_page",
        ...
        interaction: {
          name: "Diagnostics Investigations Page View",
        },
      },

      {
        name: "diseases_and_conditions_page",
        ...
        interaction: {
          name: "Diseases And Conditions Page View",
        },
      },
      {
      ...
      {
        name: "book_doctor_appointment_city_page",

        isMatch: () => {
          const pathParts = window.location.pathname.split("/").filter(Boolean);

          const matched =
            pathParts[0] === "book-doctor-appointment" &&
            pathParts.length === 2 &&
        ...
        interaction: {
          name: "Doctor By City Page",
        },
      },

      {
        name: "book_doctor_appointment_speciality_page",
        ...
        interaction: {
          name: "Doctor By Speciality Page",
        },
      },

      {
        name: "book_doctor_appointment_page",
        ...
        interaction: {
          name: "Doctor Slots And Book Appointment",
        },
      },

      {
        name: "online_doctors_page",
        ...
        interaction: {
          name: "Doctor Appointment Review Page",
        },
      },
      {
        name: "search_page",
